...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    // Validate username
    if(!filter_var(trim($_POST["username"]), FILTER_VALIDATE_EMAIL) || empty(trim($_POST["username"]))){
        $username_err = "Prosím, zadejte e-mail";
    } else {
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){
                    $username_err = "Tento e-mail je již registrován.";
                } else{
    ...
    if(empty($username_err) && empty($password_err) && empty($confirm_password_err)){
        ...
        $sql = "INSERT INTO users (username, password, discount) VALUES (?, ?, 0)";
